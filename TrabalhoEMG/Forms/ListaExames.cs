...
namespace TrabalhoEMG
{
    public partial class ListaExames : Form
    {
        ...
        public ListaExames(Cliente cliente)
        {
            ...
            InitializeComponent();
            examesFeitos = new Dados();

            this.Text = cliente.Name;


            DataView dataView = examesFeitos.DataSet.Tables[Dados.DATATABLE_EXAMES].DefaultView;
            dataView.RowFilter = string.Format("[{0}] = '{1}'", Dados.EXAMES_CLIENT_ID, cliente.Id);
           ...
           // dataGridViewExames.DataMember = Dados.DATATABLE_EXAMES;
